...
a user-defined plugin code (much in the same way that parton shower 
plugins (cf. <?php $filepath = $_GET["filepath"];
...
information about the) hard core scattering process to which hard jets are added 
...
<code>Pythia8::Merging</code> base class. It is further necessary to define the 
following functions that serve as interface to Pythia: 
...
strategy. It is permissible that this function changes <code>process</code>. In 
this case, Pythia will continue the event generation with the changed 
...
Note that because this function is the main interface between the MM plugin and 
Pythia, it is necessary to use this function to set up all the information that 
you might later need (merging weights, particle counters, etc) in this call 
already. 
...
The other main ingredient of the interface to MM plugins is a new implementation 
...
Pythia, it will need to inherit from the <code>Pythia8::MergingHooks</code> base 
class. 
 
 
<a name="method8"></a>
<p/><strong>virtual ~MyMergingHooks() &nbsp;</strong> <br/>
A destructor for your MergingHooks class. If not defined, the base class's empty 
destructor will be used. 
...
Pythia will call this function during its initialization and after all pointers 
to internal classes (e.g. to instances of the <code>Info</code> and 
<code>ParticleData</code> classes) have been set up. 
...
showering as well as multiparton interactions. It is thus necessary to properly 
start trial showers (that generate necessary no-emission probabilities), and 
for setting the correct starting conditions for parton showering of accepted 
(non-zero weight) events. 
The input <code>event</code> gives the hard process before showers and MPI are 
attempted. 
...
if the phase space for FSR/ISR/MPI is restricted (true) or unrestricted (false). 
Again, the tentative values can be overwritten. 
...
The internal implementation of <code>MergingHooks</code> in Pythia 
heavily relies on the <code>HardProcess</code> helper class. It is in principle 
...
<code>MergingHooks</code> (or the derived <code>MyMergingHooks</code> class), it 
 will need to inherit from the <code>Pythia8::HardProcess</code> base class. 
 
<a name="method24"></a>
<p/><strong>virtual ~MyHardProcess() &nbsp;</strong> <br/>
A destructor for your HardProcess class. If not defined, the base class's empty 
destructor will be used. 
   
 
<a name="method25"></a>
<p/><strong>virtual void MyHardProcess::initOnProcess( string process, ParticleData* particleData) &nbsp;</strong> <br/>
This function can be used to initialize the instance of your HardProcess class. 
In the internal Pythia implementation, this acts as a wrapper around the next 
function. 
...
may be considered as part of the core scattering process. For this, it may use 
the four next functions. 
...
cannot be part of the core scattering (e.g. because it is a final state parton, 
while the core scattering contains final state leptons only), then the function 
should return false. Else, return true to allow this candidate. Note that it 
might be possible to find multiple equally good core scattering candidates. In 
this case, all candidates should be found (with the <code>findOtherCandidates 
</code> function), and can be potentially be replaced (with 
<code>exchangeCandidates</code>). 
...
<code>doReplace=true</code>, then <code>iPos</code> will no longer be book-kept 
as part of the core scattering. An example where this functionality is helpful 
is if the input event is g g -> b b~ b  b~, and the core scattering 
